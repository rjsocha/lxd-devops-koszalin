<?php
	...
	$last=exec(sprintf("/home/socha/use_vm %s %s",$vm,$ns),$out,$res);
	if($res == 0) {
		infome('Maszyna wirtualna <b>'.$vm.'</b> została przypisana do użytkownika.');
	} else {
		warnme('Wystąpił bład przy przypisywaniu maszyny wirtualnej (' .$last. ').');
	}
	header('Location: /app.php');
