<?php
	...
	$last=exec(sprintf("/home/socha/stop_vm %s %s",$vm,$ns),$out,$res);
	if($res == 0) {
		infome('Maszyna wirtualna <b>'.$vm.'</b> została zatrzymana.');
	} else {
		warnme('Wystąpił bład przy zatrzymywaniu maszyny wirtualnej (' .$last. ').');
	}
	header('Location: /app.php');
